let exec = function() {
    ...
    input.split('\n').forEach(line => {
        ...
        let condAccepted = false
        switch (code[5]) {
            case '>':  condAccepted = registers[code[4]] >  +code[6]; break
            case '<':  condAccepted = registers[code[4]] <  +code[6]; break
            case '>=': condAccepted = registers[code[4]] >= +code[6]; break
            case '<=': condAccepted = registers[code[4]] <= +code[6]; break
            case '==': condAccepted = registers[code[4]] == +code[6]; break
        ...
        if (condAccepted) {
            if (code[1] === 'inc') {
                registers[code[0]] += +code[2]
            } else {
                registers[code[0]] -= +code[2]
            }

            if (registers[code[0]] > supermax) {
                supermax = registers[code[0]]
            }
        }
    })
    ...
    console.log('Max: ' + max)
    console.log('Global max: ' + supermax)
}

let input = `gug dec 188 if zpw >= 8
