...
class Tower  {
    constructor(name, weight, children, parent) {
        this.name = name
        this.weight = weight
    ...
    computeTotalWeight() {
        ...
        for (let child of this.towerChildren) {
    ...
    findUnbalance() {
        if (this.towerChildren.length > 0) {
            let val = this.towerChildren[0].totalWeight

            for (let child of this.towerChildren) {
                if (child.totalWeight != val) {
                    console.log('Unbalanced!!')//839 76
                }//76 196 275 435
...
const exec = () => {
    ...
    input.split('\n').map((str) => str.split(/[\s,\(\)\->]+/)).forEach((line) => {
        let tower = new Tower(line[0], +line[1])

        towers[line[0]] = tower

        if (line.length > 3) {
            tower.children.push(...line.slice(2, line.length))
        }
    })
    ...
    for (let towerKey in towers) {
        ...
        tower.children.forEach((childName) => {
            towers[childName].parent = tower
            tower.towerChildren.push(towers[childName])
        })
    }

    let topTower = null
    for (let tk in towers) {
        if (towers[tk].parent === null) {
            topTower = towers[tk] // day 07 A solution
        }
    }

    topTower.computeTotalWeight()
    topTower.findUnbalance()
    //console.log(topTower)
}
